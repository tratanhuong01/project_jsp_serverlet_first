...
@MultipartConfig
public class ProcessBia extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        processRequest(request, response);
        Part part = request.getPart("fileBia");
        ...
        String nameFile = "\\img\\imageBia\\" + user.getIdUser() + String.valueOf(idImgPost) + ".jpg";
        userDao.addAvatar(nameFile,user.getIdUser(),"LinkImageBia");
        Users users1 = userDao.getUserById(user.getIdUser());
        session.setAttribute("users", users1);
        userDao.addImagesPost(String.valueOf(idImgPost), user.getIdUser(), nameFile, String.valueOf(0));
